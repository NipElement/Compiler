...
要使用本模块，需要首先编写 `parser.y` 文件，定义语法规则以及解析这些规则形成 AST 时执行的相关动作。然后，利用 Bison 工具编译生成语法分析器源代码。源代码文件中包括一个名为 "yyparse()" 的函数。在主函数中，需要先调用词法分析器，然后调用 "yyparse()" 函数开始语法分析过程，最终生成抽象语法树。在本项目的实现示例中，我们将AST按照对应输出规则打印到控制台输出，使用 `ast->printTree()` 方法展示生成的 AST 结构。
...
通过阅读 LLVM 代码，我们编写了一段代码，将 AST 转换为 LLVM 标准的 IR。对于每一个 SysY 语言的语法结构和语义，我们都生成了相应的 LLVM IR 代码。

*代码片段 3：将 AST 转为 LLVM IR 的部分代码*
